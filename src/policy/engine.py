...
@Docs: 加载 YAML Policy 并按 deny、审批、allow 顺序评估执行决策
...
def _get_value(ctx: dict[str, Any], path: str) -> Any:
...
def _load_policies() -> list[dict[str, Any]]:
    ...
    return cast(list[dict[str, Any]], policies)


def _match_all_conditions(conditions: list[dict[str, Any]], ctx: dict[str, Any]) -> bool:
    ...
    for p in policies:
        if p.get("effect") == "deny" and _match_all_conditions(p.get("conditions") or [], ctx):
            return PolicyResult(
                ...
                matched_policy=str(p.get("name", "unnamed")),
                reason=str(p.get("description", "")),
            )
